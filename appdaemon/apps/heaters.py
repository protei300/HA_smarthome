...
class Set_heating_type(hass.Hass):
    ...
    def initialize(self):
        ...
        self.ASICS_DELTA_1 = self.settings.asics_main_1
        ...
        if self.get_state('input_boolean.asics_support_demand') == 'on':
            self.call_service('climate/turn_on', entity_id = 'climate.asics_thermostat_support')
        elif self.get_state('input_boolean.asics_support_demand') == 'off' and self.get_state('input_boolean.winter_mode') == 'off': 
            self.call_service('climate/turn_off', entity_id = 'climate.asics_thermostat_support')
        
        if self.get_state('input_boolean.asics_main_demand') == 'on':
            self.call_service('climate/turn_on', entity_id = 'climate.asics_thermostat_main')
        elif self.get_state('input_boolean.asics_main_demand') == 'off':#  and self.get_state('input_boolean.winter_mode') == 'off':
            self.call_service('climate/turn_off', entity_id = 'climate.asics_thermostat_main')
        
        if self.get_state('input_boolean.asics_main_demand_2') == 'on':
            self.call_service('climate/turn_on', entity_id = 'climate.asics_thermostat_main_2')
        elif self.get_state('input_boolean.asics_main_demand_2') == 'off':
            self.call_service('climate/turn_off', entity_id = 'climate.asics_thermostat_main_2')
        
        ##### Set ASICS TEMP in dependnce with support asics #################3
        self.listen_state(self.change_asics_temp, 'climate.asics_thermostat_temp', attribute='temperature')
    ...
    def winter_mode(self,entity, attribute, old, new, kwargs):
      if self.get_state(entity) == 'on':
          ...
          #    self.call_service('climate/turn_off', entity_id = 'climate.heater_preserve_antifreeze')
          self.call_service('climate/turn_on', entity_id = 'climate.asics_thermostat_support')
          self.call_service('input_boolean/turn_on', entity_id = 'input_boolean.asics_support_demand')
          
          #self.call_service('climate/turn_on', entity_id = 'climate.asics_thermostat_main')
          #self.call_service('input_boolean/turn_on', entity_id = 'input_boolean.asics_main_demand')
          ...
          self.call_service('climate/turn_off', entity_id = 'climate.asics_thermostat_support')
          self.call_service('input_boolean/turn_off', entity_id = 'input_boolean.asics_support_demand')
...
####### Выставляем температуру в термостатах асиках #######
    ...
    def change_asics_temp(self, entity, attribute, old, new, kwargs):
        self.call_service('climate/set_temperature', 
                                    entity_id = 'climate.asics_thermostat_main_temp',
                                    temperature = new-self.ASICS_DELTA_1)
        self.call_service('climate/set_temperature', 
                                    entity_id = 'climate.asics_thermostat_main_temp_2',
